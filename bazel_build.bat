...
REM custom setting
if "%1"=="" (
	SET ROT=%~d0/bazel_root
) else (
	SET ROT=%~d1/bazel_root
)
...
set python_path=
for /f "delims=" %%i in ('where python.bat 2^>nul') do set python_path=%%~dpi
if "%python_path%" neq "" (
	if exist "%python_path%\python3" (
		ren "%python_path%\python3" "python3.sh"
	)
	copy .\python3.exe "%python_path%" /Y
)
...
if "%link_with_wholearchive%" == "true" (
	:: link symbol "__imp_CertFreeCertificateContext"...
	echo #pragma comment(lib, "Crypt32.lib"^)>> %header_path_tmp%
